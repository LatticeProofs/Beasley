...
pub struct SumcheckProof {
    pub rounds: Vec<Vec<Fq4>>,
}

pub fn prove(
    mut tables: Vec<Vec<Fq4>>,
    deg: usize,
    combine: &dyn Fn(&[Fq4]) -> Fq4,
    mut mask: Option<&mut Masker>,
    tr: &mut Transcript,
) -> (SumcheckProof, Vec<Fq4>, Vec<Fq4>) {
    ...
    let mut vals_buf = vec![Fq4::ZERO; tables.len()];
    ...
    let finals: Vec<Fq4> = tables.iter().map(|t| t[0]).collect();
...
pub fn prove_eq_bitcheck(
    ...
    let (mut h0, mut h2) = (Fq4::ZERO, Fq::new(2) * acc);
    if let Some(m) = mask.as_deref_mut() {
    ...
    ext.extend((0..half0).map(|i| {
        let lo = Fq4::from_fq(if table.get(i) { Fq::ONE } else { Fq::ZERO });
        let hi = Fq4::from_fq(if table.get(i + half0) { Fq::ONE } else { Fq::ZERO });
        lo + r * (hi - lo)
    }));
    ...
    let mut sa: Vec<crate::simd::SoaFq4> = Vec::with_capacity(mid + 1);
    for k in 0..=mid {
        sa.push(crate::simd::eq_table_mont(&tau[mid - k..mid]));
    }
    let mut soa = crate::simd::SoaFq4::from_aos(ext);
...
pub fn verify_eq_bitcheck(
    ...
    for (j, evals) in proof.rounds.iter().enumerate() {
        ...
        let t = tau[j];
        let h1 = (c * a.inv() - (Fq4::ONE - t) * h0) * t.inv();
        tr.absorb_fq4(h0);
        tr.absorb_fq4(h2);
        let r = tr.challenge_fq4();
        r_point.push(r);
        let hr = lagrange_eval(&[h0, h1, h2], r);
        a = a * (t * r + (Fq4::ONE - t) * (Fq4::ONE - r));
        c = a * hr;
    }
    Some((c, r_point))
}
...
pub fn prove_product2(
    ...
    mut mask: Option<&mut Masker>,
    tr: &mut Transcript,
) -> (SumcheckProof, Vec<Fq4>) {
    let nv = b.len().trailing_zeros() as usize;
    assert_eq!(a_base.len(), b.len());
    let mut rounds = Vec::with_capacity(nv);
    let mut r_point = Vec::with_capacity(nv);
    let mut a_ext: Vec<Fq4> = Vec::new();

    for j in 0..nv {
        let half = 1usize << (nv - 1 - j);
        let (mut g0, mut g2) = (Fq4::ZERO, Fq4::ZERO);
        if j == 0 {
            for cell in 0..half {
                let alo = a_base[cell];
        ...
        if j == 0 {
            a_ext = (0..half)
                .map(|i| {
                    let lo = Fq4::from_fq(a_base[i]);
                    lo + r * (Fq4::from_fq(a_base[i + half]) - lo)
                })
                .collect();
        } else {
            for i in 0..half {
...
pub fn prove_product2_factored(
    ...
    for j in 0..nv_k {
        ...
        let (mut g0, mut g2) = (Fq4::ZERO, Fq4::ZERO);
        for k in 0..half_k {
            let (mut row0, mut row2) = (Fq4::ZERO, Fq4::ZERO);
            if j == 0 {
                let lo_base = k * s_len;
        ...
        if j == 0 {
            a_ext.extend((0..half_cells).map(|i| {
                let lo = Fq4::from_fq(if a_base.get(i) { Fq::ONE } else { Fq::ZERO });
                let hi = Fq4::from_fq(if a_base.get(i + half_cells) { Fq::ONE } else { Fq::ZERO });
                lo + r * (hi - lo)
            }));
        } else {
    ...
    if nv_k == 0 {
        a_ext.extend((0..a_base.len()).map(|i| {
            Fq4::from_fq(if a_base.get(i) { Fq::ONE } else { Fq::ZERO })
        }));
    }
    let mut b: Vec<Fq4> = apow.iter().map(|&p| lg[0] * p).collect();
    for _ in nv_k..nv {
...
pub fn prove_batched_w(
    ...
    let mut sa: Vec<Vec<Fq4>> = Vec::with_capacity(mid + 1);
    for k in 0..=mid {
        sa.push(eq_table(&tau0[mid - k..mid]));
    }

    let apow_soa = crate::simd::SoaFq4::from_aos(apow);
    let eb_soa = crate::simd::SoaFq4::from_aos(&eb);
    let mut wsoa: Option<crate::simd::SoaFq4> = None;

    let w = scratch;
    w.clear();
    let mut r_point = Vec::with_capacity(nv_w);
    let mut rounds = Vec::with_capacity(nv_w);
    let mut b: Vec<Fq4> = Vec::new();
    let mut eq_prefix = Fq4::ONE;

    for round in 0..nv_w {
        let half = 1usize << (nv_w - 1 - round);
        let eqsuf_mat: Vec<Fq4>;
        let ea: &[Fq4] = if round < mid {
            &sa[mid - 1 - round]
        } else {
        ...
        let read_w = |cell: usize, w: &[Fq4]| -> (Fq4, Fq4, Fq4) {
            let (lo, hi) = if round == 0 {
                (
                    Fq4::from_fq(if zw.get(cell) { Fq::ONE } else { Fq::ZERO }),
                    Fq4::from_fq(if zw.get(cell + half) { Fq::ONE } else { Fq::ZERO }),
                )
            } else {
                (w[cell], w[cell + half])
            };
        ...
        let (mut gf2_0, mut gf2_2, mut gf2_3) = (Fq4::ZERO, Fq4::ZERO, Fq4::ZERO);
        let (mut q3_0, mut q3_2, mut q3_3) = (Fq4::ZERO, Fq4::ZERO, Fq4::ZERO);

        if round < nv_k && round >= 1 {
            let ea_soa = crate::simd::SoaFq4::from_aos(ea);
            ...
            for cell in 0..half {
                ...
                let e = eq_rem(cell);
                q3_0 = q3_0 + e * (w0 * (w0 - Fq4::ONE));
                q3_2 = q3_2 + e * (w2 * (w2 - Fq4::ONE));
                q3_3 = q3_3 + e * (w3 * (w3 - Fq4::ONE));
            }
        }

        let t = tau0[round];
        let eq1_0 = Fq4::ONE - t;
        let slope = t + t - Fq4::ONE;
        ...
        r_point.push(r);
        eq_prefix = eq_prefix * (t * r + (Fq4::ONE - t) * (Fq4::ONE - r));
        ...
        if round == 0 {
            if nv_k >= 2 {
                ...
                w.extend((0..half).map(|cell| {
                    let lo = Fq4::from_fq(if zw.get(cell) { Fq::ONE } else { Fq::ZERO });
                    let hi = Fq4::from_fq(if zw.get(cell + half) { Fq::ONE } else { Fq::ZERO });
                    lo + r * (hi - lo)
                }));
            }
        } else if round < nv_k {
...
pub fn lagrange_eval(evals: &[Fq4], x: Fq4) -> Fq4 {
    let deg = evals.len() - 1;
    let mut acc = Fq4::ZERO;
    for i in 0..=deg {
        let mut num = Fq4::ONE;
        let mut den = Fq4::ONE;
        let xi = Fq4::from_u64(i as u64);
        for k in 0..=deg {
            if k == i {
                continue;
            }
            let xk = Fq4::from_u64(k as u64);
            num = num * (x - xk);
            den = den * (xi - xk);
        }
...
mod tests {
    ...
    fn product_sumcheck_roundtrip() {
        ...
        let nv = 6;
        let a: Vec<Fq4> = (0..1 << nv).map(|_| rng.next_fq4()).collect();
        let b: Vec<Fq4> = (0..1 << nv).map(|_| rng.next_fq4()).collect();
        let claim = a.iter().zip(&b).fold(Fq4::ZERO, |s, (&x, &y)| s + x * y);

        let combine = |v: &[Fq4]| v[0] * v[1];
    ...
    fn wrong_claim_rejected() {
        ...
        assert!(verify(claim + Fq4::ONE, nv, 2, &proof, &mut tr_v).is_none());
    }

    #[test]
    fn lagrange_recovers_polynomial() {
        let p = |x: u64| Fq4::from_u64(3 * x * x + 2 * x + 7);
        let evals = vec![p(0), p(1), p(2)];
        assert_eq!(lagrange_eval(&evals, Fq4::from_u64(5)), p(5));
    }
    ...
    fn eq_bitcheck_roundtrip() {
        ...
        let (expect, r_v) =
            verify_eq_bitcheck(Fq4::ZERO, &tau, &proof, &mut tr_v).expect("verify");
        assert_eq!(r_p, r_v);
        let lifted: Vec<Fq4> = table.iter().map(|&x| Fq4::from_fq(x)).collect();
        let t_r = mle_eval(&lifted, &r_v);
        assert_eq!(expect, eq_eval(&tau, &r_v) * t_r * (t_r - Fq4::ONE));
    }
    ...
    fn eq_bitcheck_rejects_tampered_round() {
        ...
        proof.rounds[2][1] = proof.rounds[2][1] + Fq4::ONE;

        use crate::mle::{eq_eval, mle_eval};
        let mut tr_v = Transcript::new("bc-test2");
        let (expect, r_v) =
            verify_eq_bitcheck(Fq4::ZERO, &tau, &proof, &mut tr_v).expect("rounds well-formed");
        let lifted: Vec<Fq4> = table.iter().map(|&x| Fq4::from_fq(x)).collect();
        let t_r = mle_eval(&lifted, &r_v);
        assert_ne!(expect, eq_eval(&tau, &r_v) * t_r * (t_r - Fq4::ONE));
    }
    ...
    fn product2_factored_matches_materialized() {
        ...
        let lg: Vec<Fq4> = (0..1 << nv_k).map(|_| rng.next_fq4()).collect();
        let apow: Vec<Fq4> = (0..s_len).map(|_| rng.next_fq4()).collect();
        let b: Vec<Fq4> = (0..total).map(|i| lg[i / s_len] * apow[i % s_len]).collect();
    ...
    fn batched_w_matches_bruteforce() {
        ...
        let w_fq4: Vec<Fq4> =
            w_bits.iter().map(|&b| Fq4::from_u64(b as u64)).collect();
        let zw = PackedBits::from_bits(&w_bits, total);
        let lg: Vec<Fq4> = (0..1 << nv_k).map(|_| rng.next_fq4()).collect();
        let apow: Vec<Fq4> = (0..s_len).map(|_| rng.next_fq4()).collect();
        let tau0: Vec<Fq4> = (0..nv_w).map(|_| rng.next_fq4()).collect();
        let lambda = rng.next_fq4();

        let mut claim2 = Fq4::ZERO;
        for cell in 0..total {
        ...
        let mut bb: Vec<Fq4> = Vec::new();
        let mut tr_sim = Transcript::new("bw");
        let mut sim_r: Vec<Fq4> = Vec::new();
        let mut claim_sim = lambda * claim2;
        let mut eqpre = Fq4::ONE;
        for round in 0..nv_w {
            let half = 1 << (nv_w - 1 - round);
            let t = tau0[round];
            let eqrem = |cell: usize| -> Fq4 {
                let rem = nv_w - 1 - round;
                let mut e = Fq4::ONE;
                for i in 0..rem {
                    let bit = (cell >> (rem - 1 - i)) & 1;
                    let tt = tau0[round + 1 + i];
                    e = e * if bit == 1 { tt } else { Fq4::ONE - tt };
                }
                e
            };
            let gx = |x: Fq4, wb: &[Fq4], lgb: &[Fq4], bb: &[Fq4]| -> Fq4 {
                let mut g = Fq4::ZERO;
                let eq1x = t * x + (Fq4::ONE - t) * (Fq4::ONE - x);
                for cell in 0..half {
                    let wlo = wb[cell];
                    ...
                    g = g + lambda * f2 + eqpre * eq1x * eqrem(cell) * wx * (wx - Fq4::ONE);
                }
                g
            };
            let bg0 = gx(Fq4::ZERO, &wb, &lgb, &bb);
            let bg1 = gx(Fq4::ONE, &wb, &lgb, &bb);
            let bg2 = gx(Fq4::from_u64(2), &wb, &lgb, &bb);
            let bg3 = gx(Fq4::from_u64(3), &wb, &lgb, &bb);
            ...
            sim_r.push(r);
            eqpre = eqpre * (t * r + (Fq4::ONE - t) * (Fq4::ONE - r));
        ...
        let sim_final =
            lambda * bb[0] * wb[0] + eq_eval(&tau0, &sim_r) * wb[0] * (wb[0] - Fq4::ONE);

        let mut tr_v = Transcript::new("bw");
        let (e_b, r_v) = verify_batched_w(lambda * claim2, nv_w, &proof, &mut tr_v).expect("v");
        assert_eq!(r, r_v);
        assert_eq!(sim_r, r_v);
        assert_eq!(e_b, sim_final, "e_b vs sim-final（verify 折疊）");
    ...
    fn batched_w_reference(
        ...
        for round in 0..nv_w {
            ...
            let eqrem = |cell: usize| -> Fq4 {
                let rem = nv_w - 1 - round;
                (0..rem).fold(Fq4::ONE, |e, i| {
                    let tt = tau0[round + 1 + i];
                    e * if (cell >> (rem - 1 - i)) & 1 == 1 { tt } else { Fq4::ONE - tt }
                })
            };
            let gx = |x: Fq4, w: &[Fq4], lg: &[Fq4], b: &[Fq4]| -> Fq4 {
                let eq1x = t * x + (Fq4::ONE - t) * (Fq4::ONE - x);
                (0..half).fold(Fq4::ZERO, |g, cell| {
                    ...
                    g + lambda * bx * wx + eqpre * eq1x * eqrem(cell) * wx * (wx - Fq4::ONE)
                })
            };
            let (g0, g2, g3) = (
                gx(Fq4::ZERO, &w, &lg, &b),
                gx(Fq4::from_u64(2), &w, &lg, &b),
                gx(Fq4::from_u64(3), &w, &lg, &b),
            );
            tr.absorb_fq4(g0);
            tr.absorb_fq4(g2);
            tr.absorb_fq4(g3);
    ...
    fn batched_w_reference_matches_prover_on_honest_bits() {
        ...
        let w: Vec<Fq4> = bits.iter().map(|&b| Fq4::from_u64(b as u64)).collect();
        let zw = PackedBits::from_bits(&bits, total);
        let lg: Vec<Fq4> = (0..1 << nv_k).map(|_| rng.next_fq4()).collect();
        let apow: Vec<Fq4> = (0..s_len).map(|_| rng.next_fq4()).collect();
        let tau0: Vec<Fq4> = (0..nv_k + nv_l).map(|_| rng.next_fq4()).collect();
        let lambda = rng.next_fq4();
    ...
    fn non_binary_w_is_rejected(bad_cell: usize, bad_val: u64, seed: u64) {
        ...
        let mut w: Vec<Fq4> =
            (0..total).map(|_| Fq4::from_u64(rng.next_bool() as u64)).collect();
        w[bad_cell] = Fq4::from_u64(bad_val);
        let lg: Vec<Fq4> = (0..1 << nv_k).map(|_| rng.next_fq4()).collect();
        let apow: Vec<Fq4> = (0..s_len).map(|_| rng.next_fq4()).collect();
        let tau0: Vec<Fq4> = (0..nv_w).map(|_| rng.next_fq4()).collect();
        let lambda = rng.next_fq4();
        ...
        let claim2 = (0..total)
            .fold(Fq4::ZERO, |a, c| a + lg[c / s_len] * apow[c % s_len] * w[c]);
        ...
        let (e_b, r_v) =
            verify_batched_w(lambda * claim2, nv_w, &proof, &mut tr_v).expect("格式合法");
        assert_eq!(r_p, r_v);
        ...
        let expect =
            lambda * lg_at * ap_at * w_at + eq_eval(&tau0, &r_v) * w_at * (w_at - Fq4::ONE);
        assert_ne!(e_b, expect, "非二元的 W（cell {bad_cell} = {bad_val}）竟然通過 SC3");
    }
    ...
    fn product2_roundtrip_and_wrong_claim() {
        ...
        assert_eq!(r_p, r_v);
        let lifted: Vec<Fq4> = a.iter().map(|&x| Fq4::from_fq(x)).collect();
        assert_eq!(expect, mle_eval(&lifted, &r_v) * mle_eval(&b, &r_v));

        let mut tr_w = Transcript::new("p2-test");
        let (expect_w, r_w) =
            verify_product2(claim + Fq4::ONE, nv, &proof, &mut tr_w).expect("well-formed");
...
mod lagrange3 {
    ...
    #[test]
    fn lagrange_deg3() {
        let p = |x: u64| Fq4::from_u64(2*x*x*x + 3*x*x + 5*x + 7);
        let e = vec![p(0), p(1), p(2), p(3)];
        assert_eq!(lagrange_eval(&e, Fq4::from_u64(9)), p(9));
        assert_eq!(lagrange_eval(&e, Fq4::from_u64(2)), p(2));
    }
}
...
impl Masker {
    pub fn new(rng: &mut crate::rng::CsRng, nv: usize, deg: usize) -> Self {
        let coef: Vec<Vec<Fq4>> =
            (0..nv).map(|_| (0..=deg).map(|_| rng.next_fq4()).collect()).collect();
        let ends: Vec<Fq4> = coef
            .iter()
            .map(|c| {
                let g0 = c[0];
                let g1 = c.iter().fold(Fq4::ZERO, |a, &v| a + v);
                g0 + g1
            })
            .collect();
        let mut suffix = vec![Fq4::ZERO; nv + 1];
        for i in (0..nv).rev() {
        ...
        Masker { coef, suffix, nv, deg, rho: Fq4::ONE, prefix: Fq4::ZERO, pow2 }
    }
    ...
    pub fn coeffs_flat(&self) -> impl Iterator<Item = Fq4> + '_ {
        self.coef.iter().flat_map(|c| c.iter().copied())
    }

    pub fn set_rho(&mut self, rho: Fq4) {
        self.rho = rho;
    }

    fn gi(&self, i: usize, p: Fq4) -> Fq4 {
        self.coef[i].iter().rev().fold(Fq4::ZERO, |acc, &c| acc * p + c)
    }

    pub fn total_plain(&self) -> Fq4 {
        if self.nv == 0 {
            return Fq4::ZERO;
        }
    ...
    pub fn total_eq(&self, tau: &[Fq4]) -> Fq4 {
        debug_assert_eq!(tau.len(), self.nv);
        (0..self.nv).fold(Fq4::ZERO, |acc, i| {
            let g0 = self.coef[i][0];
            let g1 = self.coef[i].iter().fold(Fq4::ZERO, |a, &v| a + v);
            acc + (Fq4::ONE - tau[i]) * g0 + tau[i] * g1
        })
    }

    pub fn round_plain(&self, round: usize, pts: &[u64]) -> Vec<Fq4> {
        ...
        pts.iter()
            .map(|&p| {
                let g = self.gi(round, Fq4::from_u64(p));
                self.rho * (lead * (g + self.prefix) + tail)
            })
            .collect()
    }

    pub fn round_eq(&self, round: usize, tau: &[Fq4], pts: &[u64]) -> Vec<Fq4> {
        let tail = ((round + 1)..self.nv).fold(Fq4::ZERO, |acc, i| {
            let g0 = self.coef[i][0];
            let g1 = self.coef[i].iter().fold(Fq4::ZERO, |a, &v| a + v);
            acc + (Fq4::ONE - tau[i]) * g0 + tau[i] * g1
        });
        pts.iter()
            .map(|&p| self.rho * (self.gi(round, Fq4::from_u64(p)) + self.prefix + tail))
            .collect()
    }

    pub fn fold(&mut self, round: usize, r: Fq4) {
        self.prefix = self.prefix + self.gi(round, r);
    }

    pub fn eval(&self) -> Fq4 {
        self.prefix
    }

    pub fn weights_eval(nv: usize, deg: usize, r: &[Fq4]) -> Vec<Fq4> {
    ...
    pub fn weights_total_plain(nv: usize, deg: usize) -> Vec<Fq4> {
        let mut lead = Fq4::ONE;
    ...
    pub fn weights_total_eq(nv: usize, deg: usize, tau: &[Fq4]) -> Vec<Fq4> {
        let mut out = Vec::with_capacity(nv * (deg + 1));
        for &t in tau.iter().take(nv) {
            for k in 0..=deg {
                out.push(if k == 0 { Fq4::ONE } else { t });
            }
        }
        out
    }
...
mod mask_tests {
    ...
    fn brute(m: &Masker, x: &[Fq4]) -> Fq4 {
        (0..m.nv).fold(Fq4::ZERO, |a, i| a + m.gi(i, x[i]))
    }
    ...
    fn total_plain_matches_brute_force() {
        for (nv, deg) in [(1usize, 2usize), (3, 3), (4, 2), (5, 3)] {
            ...
            for mask in 0..(1u32 << nv) {
                let x: Vec<Fq4> = (0..nv)
                    .map(|i| Fq4::from_u64(((mask >> i) & 1) as u64))
                    .collect();
                s = s + brute(&m, &x);
            }
    ...
    fn total_eq_matches_brute_force() {
        for (nv, deg) in [(1usize, 2usize), (3, 3), (4, 2)] {
            ...
            for cell in 0..(1usize << nv) {
                let x: Vec<Fq4> = (0..nv)
                    .map(|i| Fq4::from_u64(((cell >> (nv - 1 - i)) & 1) as u64))
                    .collect();
    ...
    fn round_evals_telescope_to_the_total() {
        ...
        for (nv, deg) in [(1usize, 2usize), (3, 3), (5, 2), (6, 3)] {
            ...
            assert_eq!(claim, m.eval(), "plain 收尾：最後的 claim 必須是 g(r)");
            ...
            for j in 0..nv {
                ...
                let lhs = a * ((Fq4::ONE - tau[j]) * v[0] + tau[j] * v[1]);
                ...
                a = a * (tau[j] * r + (Fq4::ONE - tau[j]) * (Fq4::ONE - r));
                claim = a * hr;
                m.fold(j, r);
            }
        }
    ...
    fn linear_weights_match_the_closed_forms() {
        ...
        for (nv, deg) in [(1usize, 2usize), (3, 3), (5, 2)] {
            ...
            let dot = |w: &[Fq4], m: &Masker| -> Fq4 {
                m.coeffs_flat().zip(w).fold(Fq4::ZERO, |a, (c, &x)| a + c * x)
            };
