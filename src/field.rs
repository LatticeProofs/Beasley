...
const _: () = assert!(FQ_BYTES == 8, "Fq must be 8 bytes");
...
mod tests {
    ...
    fn generator_has_full_order() {
        ...
        for p in factors {
            assert_ne!(g.pow((Q - 1) / p), Fq::ONE, "GENERATOR = {GENERATOR} is not a generator w.r.t. p = {p}");
        }
    ...
    fn roots_of_unity() {
        assert_eq!((Q - 1) % 4, 0);
        assert_ne!((Q - 1) % 8, 0, "v2(q-1) should be 2");
        let w = Fq::root_of_unity(2);
        assert_eq!(w, Fq::new(Q - 1));
        assert_eq!(w.pow(2), Fq::ONE);
        assert_eq!(Fq::root_of_unity(4).pow(4), Fq::ONE);
        assert_ne!((Q - 1) % (2 * crate::ring::N as u64), 0, "if divisible, use a direct NTT instead");
    }
    ...
    fn q_is_five_mod_eight() {
        ...
        assert_eq!(Fq::new(Q - 1).pow((Q - 1) / 2), Fq::ONE, "-1 should be a quadratic residue");
    }
